...
let printMessage = (message = 'Сообщение отсутствуе') => message;
...
function thirdFunction(f = 3, g = 3) {
	//let z = f;
	//let p = g;
	return f * g;
}

console.log(thirdFunction(f, g)); //Выведет 6
...
console.log(thirdFunction(f)); //Выведет 12
console.log(thirdFunction(f, g)); //Выведет 30
...
let greetMessage = (guestName = 'Гость') => 'Привет' + ', ' + guestName + '!';

console.log (greetMessage(guestName));
...
let calculateProduct = function(f, g) {
...
console.log('Прияиель, тебе уже ' + calculateAge(todayYear, friendYearOfBirth) + ' лет.');
...
const yourYearBirth = 1;

let yearNow = 2023;

let calculateAgeNow = (x, y) => {
	let age = (x - y);
	return age;
}

console.log('Вам сейчас ' + calculateAgeNow(yearNow, yourYearBirth) + ' лет.');
...
let resultPower = degreeNum(num, degree);
console.log('Результат ' + resultPower);